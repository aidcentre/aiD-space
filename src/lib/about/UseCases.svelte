	...
	<div class="flex min-h-screen flex-wrap px-4 my-30 md:my-40 lg:my-60">
		...
		<div class="-mt-6 grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-12 max-w-[1920px] mx-auto">
			...
			<div class="col-span-4 md:col-start-8">
				<div class="flex flex-col gap-8 lg:gap-20 xl:gap-32 2xl:gap-40">
					{#each useCases_data as item}
						<div class="flex flex-col gap-2 md:gap-4">
							<div class="text-lg font-bold text-off-black dark:text-white lg:text-xl transition-colors duration-800 ease-out-expo">{item.title}</div>
							<p class="md:text-md font-family-serif text-sm text-off-black dark:text-white lg:text-lg transition-colors duration-800 ease-out-expo">
								{item.content}
							</p>
						</div>
					{/each}
