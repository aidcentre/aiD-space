...
>
	...
	<div class="flex items-center flex-wrap">
		{#if dateLabel}
			<span class="font-family-mono text-xs md:text-sm text-off-black px-4 md:px-5 lg:px-6 py-3 lg:py-4 group-hover:text-white transition-colors duration-800 ease-out-expo">{dateLabel}</span>
		{/if}
		{#if typeLabel}
			<span class="font-family-mono text-xs md:text-sm text-off-black border-l border-light-grey px-4 md:px-5 lg:px-6 py-3 lg:py-4 group-hover:text-white group-hover:border-darker-grey transition-colors duration-800 ease-out-expo">{typeLabel}</span>
		{/if}
		{#if locationLabel}
			<span class="font-family-mono text-xs md:text-sm text-off-black border-l border-light-grey px-4 md:px-5 lg:px-6 py-3 lg:py-4 group-hover:text-white group-hover:border-darker-grey transition-colors duration-800 ease-out-expo">{locationLabel}</span>
		{/if}
	</div>
</a>
