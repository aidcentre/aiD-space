...
def load_dataframe(filename: str) -> pd.DataFrame:
    ...
    """
    with resources.files("aid_expertise_search.datasets").joinpath(filename).open("rb") as f:
...
def get_scientific_area(query: Union[str, List[tuple[str,str]]]) -> AreaOfQuery:
    ...
    if type(query) == str:
        ...
        prompt.extend([{"role": messager, "content": content} for messager, content in query])
...
def read_pdf_to_chunks(filename: str, chunk_lim: int = -1) -> tuple[str, List[str]]:
    ...
    """
    if chunk_lim == -1:
        pages = [page['text'] for page in pymupdf4llm.to_text(pymupdf.open(filename), page_chunks=True, use_ocr=False)]
        return ('\n'.join(pages), pages)
        ...
        if chunk_lim < 100:
            ...
            split_text = [text for text in full_text.split("\n\n") if text not in ("", " ", "_", "-")]
            chunks = []
            current_chunk = ""
...
def retrieval(df, vector, n_min = 3, n_max = -1) -> pd.DataFrame:
    ...
    df['cosine'] = df['chunk_embeddings'].apply(lambda x: max_cosine_for_document(x, vector))
    ...
    if n_max != -1:
        smallest_cosine_allowed = np.sort(df['cosine'])[::-1][min(n_max-1, n_docs-1)]
...
def find_mentioned_researchers(text_answer: str, most_relevant_researchers: List[tuple[str, float]], mode: str = 'mentioned_most_relevant') -> List[str]:
    """
