...
from aid_expertise_search.clients import weak_client, medium_client, strong_client, embedding_model
...
def SearchAndEvaluateNode(state: State) -> State:
    ...
    if scientific_area.specific_area.lower() == "other":
        vector = embedding_model.encode(state.get('query'))
    else:
        vector = embedding_model.encode(state.get('scientific_area').specific_area)
